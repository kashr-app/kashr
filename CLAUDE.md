...
### State Management
* **Built-in Solutions:** Prefer Flutter's built-in state management solutions for simple cases.
* **Flutter Bloc:** For more complex state management use `flutter_bloc` but prefer cubits.
...
* **Provider:** If a dependency injection solution beyond manual constructor
  ...
  coupling. For cubits and repositories always prefer using `provider` and use
...
* **GoRouter:** Use the `go_router` package for declarative navigation, deep
  linking, and web support. Only use typed routes and use `go_router_builder`.
...
* **Structured Logging:** Use the `log` function from `dart:developer` for
  ...
  ```
* **Addtional Logging:** Use the `logger` package for information (`log.i()`) and critical erros (`log.e(....)`).
